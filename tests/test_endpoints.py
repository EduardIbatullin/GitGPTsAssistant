...
from app.domain.models import FileContentResponse
...
class DummyFileService:
    ...
    async def get_file_content(self, repo: str, path: str) -> FileContentResponse:
        return FileContentResponse(path=path, content=f"Content of {path}", encoding="utf-8")
...
class DummyGitInfoService:
    ...
    async def list_pull_requests(self, repo: str) -> list:
        return [
            {"title": "Fix bug", "number": 1},
            {"title": "Add feature", "number": 2}
        ]

    async def list_issues(self, repo: str) -> list:
        return [
            {"title": "Crash on load", "number": 3},
            {"title": "UI glitch", "number": 4}
        ]
...
class ErrorFileService:
    ...
    async def get_repo_structure(self, repo: str): raise self.exc
    async def get_file_content(self, repo: str, path: str): raise self.exc
    async def create_file(self, repo: str, path: str, filename: str, content: str, message: str): raise self.exc
    async def update_file(self, repo: str, path: str, filename: str, content: str, message: str): raise self.exc
    async def delete_file(self, repo: str, path: str, filename: str, message: str): raise self.exc
...
@pytest.mark.parametrize("exc,method,endpoint,payload,exp_status,exp_detail", [
    (InvalidRepositoryError("Репозиторий 'no-repo' не найден"), "GET", "/repos/no-repo/file?path=any.txt", None, 404, "Репозиторий 'no-repo' не найден"),
    (ResourceNotFoundError("Файл 'nofile.txt' не найден"), "GET", "/repos/existing-repo/file?path=nofile.txt", None, 404, "Файл 'nofile.txt' не найден"),
    (InvalidRepositoryError("Репозиторий 'no-repo' не найден"), "PUT", "/repos/no-repo/file", {"path": "", "filename": "f.txt", "content": "x", "message": "m"}, 404, "Репозиторий 'no-repo' не найден"),
    (ResourceNotFoundError("Файл 'sub/f.txt' не найден"), "PUT", "/repos/existing-repo/file", {"path": "sub", "filename": "f.txt", "content": "x", "message": "m"}, 404, "Файл 'sub/f.txt' не найден"),
    (InvalidRepositoryError("Репозиторий 'no-repo' не найден"), "DELETE", "/repos/no-repo/file", {"path": "", "filename": "f.txt", "message": "m"}, 404, "Репозиторий 'no-repo' не найден"),
    (ResourceNotFoundError("Файл 'sub/f.txt' не найден"), "DELETE", "/repos/existing-repo/file", {"path": "sub", "filename": "f.txt", "message": "m"}, 404, "Файл 'sub/f.txt' не найден")
])
